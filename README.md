...
```
grep "cd build/Foil\|cd build/Kelvin\|bin/picongpu \|calculation" output/hal-sleeptimes/run_* > results.txt
```

(with the one-build-per-example layout the `cd` paths are absolute, so use a
pattern like `cd .*build/(FoilLCT|KelvinHelmholtz)` instead), then
